...
import ToxinasTab from '../components/ToxinasTab'
import { supabase, useAuth } from '@gestor/core'
import { ArrowLeft, FileText, DollarSign, Trash2, UserCircle, Calendar, ClipboardList, Syringe, Plus } from 'lucide-react'
...
export default function PacienteDetalhes() {
  ...
  useEffect(() => {
    ...
    const validTabs = ['dados', 'prontuarios', 'prontuario', 'pagamentos', 'sessoes-agendadas', 'toxinas']
  ...
  const handleDeletePaciente = async () => {
    ...
    if (!confirm(`Tem certeza que deseja excluir o paciente "${paciente.nome_completo}"?\n\nEsta ação não pode ser desfeita e todos os dados relacionados (avaliações, prontuários e pagamentos) serão removidos.`)) {
      return
    }
    ...
    try {
      ...
      // Deletar sessões de toxina
      await supabase.from('sessoes_toxinas').delete().eq('paciente_id', id)
  ...
  // Tabs específicos para esteticista
  const tabs = [
    ...
    { id: 'toxinas', label: 'Toxinas', icon: Syringe },
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="p-4 sm:p-6">
            ...
            {activeTab === 'prontuarios' && <ProntuariosEsteticosTab pacienteId={id!} paciente={paciente} />}
            {activeTab === 'prontuario' && <ProntuarioTab pacienteId={id!} paciente={paciente} />}
            {activeTab === 'toxinas' && <ToxinasTab pacienteId={id!} paciente={paciente} />}
