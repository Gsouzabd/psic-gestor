...
export interface Session {
  ...
  confirmada_pelo_paciente?: boolean | null
...
interface CalendarProps {
  ...
  onMultipleSessionsClick?: (date: Date, sessions: Session[]) => void
  blockedDays?: string[] // Array de datas no formato YYYY-MM-DD
}
...
export default function Calendar({ sessions = [], onEventClick, onDayClick, onMultipleSessionsClick, blockedDays = [] }: CalendarProps) {
      ...
      days.push(
        <div
          ...
          className={`min-h-[60px] sm:min-h-[80px] md:min-h-[100px] border border-gray-200 p-1 sm:p-1.5 md:p-2 ${
            isCurrentMonth ? (isBlocked ? 'bg-gray-200' : 'bg-white') : 'bg-gray-50'
          } ${isDayToday ? 'ring-2 ring-primary' : ''} ${
            onDayClick && isCurrentMonth && !isBlocked ? 'cursor-pointer hover:bg-gray-50' : ''
          } ${isBlocked ? 'opacity-60' : ''}`}
          onClick={() => {
            if (onDayClick && isCurrentMonth && !isBlocked) {
        ...
        >
          <div className={`text-xs sm:text-sm font-medium mb-0.5 sm:mb-1 ${
            ...
            isCurrentMonth ? (isBlocked ? 'text-gray-500 line-through' : 'text-gray-900') : 'text-gray-400'
          }`}>
            {format(currentDay, 'd')}
          </div>
