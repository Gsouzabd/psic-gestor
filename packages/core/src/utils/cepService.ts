 ...
 * @returns {Promise<CepResult>} Dados do endereço ou null se não encontrado
 */
export async function buscarCEP(cep: string): Promise<CepResult> {
  ...
  try {
    ...
    if (error instanceof Error && (error.message === 'CEP não encontrado' || error.message === 'CEP deve conter 8 dígitos')) {
      throw error
    }
