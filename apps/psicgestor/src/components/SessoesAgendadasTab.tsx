...
import { supabase, marcarComparecimento, deleteFutureRecurringAppointments, RecurrenceActionModal, notifyPatient, useToast, Modal } from '@gestor/core'
...
interface SessoesAgendadasTabProps {
  pacienteId: string
  paciente: any
}

export default function SessoesAgendadasTab({ pacienteId, paciente }: SessoesAgendadasTabProps) {
  const { success, error: showError } = useToast()
  const [loading, setLoading] = useState(true)
  const [sessoesAgendadas, setSessoesAgendadas] = useState<any[]>([])
  const [sessoesFiltradas, setSessoesFiltradas] = useState<any[]>([])
  const [filtroStatus, setFiltroStatus] = useState<'todas' | 'agendadas' | 'confirmadas' | 'canceladas'>('todas')
  const [showRecurrenceModal, setShowRecurrenceModal] = useState(false)
  const [pendingSessao, setPendingSessao] = useState<any>(null)
  const [notifyingSessaoId, setNotifyingSessaoId] = useState<string | null>(null)
  const [notificationUseApelidoMap, setNotificationUseApelidoMap] = useState<Record<string, boolean>>({})
  
  // Modal de Criar Pagamento
  const [showCriarPagamentoModal, setShowCriarPagamentoModal] = useState(false)
  const [sessaoParaPagamento, setSessaoParaPagamento] = useState<any>(null)
  ...
  const aplicarFiltro = (sessoes: any[], filtro: string) => {
  ...
  const handleCriarPagamento = (sessao: any) => {
  ...
  const handleMarcarComparecimento = async (sessao: any, compareceu: boolean) => {
    try {
  ...
  const handleDeleteClick = (sessao: any) => {
  ...
  const handleDeleteRecurrence = async (deleteAllSeries: boolean) => {
  ...
  const handleDeleteSessao = async (sessaoId: string, isRecurring = false) => {
