...
import { supabase, Modal, SessionCard, ImageUpload } from '@gestor/core'
...
export default function ProntuarioTab({ pacienteId, paciente }: ProntuarioTabProps) {
  ...
  const [formData, setFormData] = useState({
    data: '',
    hora: '',
    compareceu: true as boolean | null,
    anotacoes: '',
    valor_sessao: paciente?.valor_sessao || '',
    desconto: '0',
    imagens_urls: [] as string[]
  })
  ...
  const fetchSessoes = async () => {
    try {
      // Buscar apenas prontuários de sessões que já aconteceram (compareceu != null)
      const { data, error } = await supabase
        .from('prontuarios')
        .select('*')
        .eq('paciente_id', pacienteId)
        .not('compareceu', 'is', null) // Apenas sessões com compareceu = true ou false
  ...
  const handleEdit = async (sessao: any) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingSessao) {
        ...
        // Criar novo prontuário (apenas sessões que já aconteceram)
