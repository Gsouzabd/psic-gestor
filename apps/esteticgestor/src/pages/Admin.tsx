...
interface Psicologo {
  ...
  email: string
  role: 'psicologo' | 'esteticista' | 'admin_master'
...
export default function Admin() {
  ...
  const [resettingPasswordFor, setResettingPasswordFor] = useState<Psicologo | null>(null)
  ...
  const handleEdit = (psicologo: Psicologo) => {
    setEditingPsicologo(psicologo)
    setFormData({
      nome_completo: psicologo.nome_completo || '',
      email: psicologo.email || '',
      role: (psicologo.role === 'esteticista' ? 'esteticista' : 'psicologo') as 'psicologo' | 'esteticista'
    })
  ...
  const handleDelete = async (psicologo: Psicologo) => {
  ...
  const handleResetPassword = async (psicologo: Psicologo) => {
  ...
  const togglePasswordVisibility = (psicologoId: string) => {
  ...
  const loadEvolutionApiConfig = async () => {
    try {
      ...
      const config: EvolutionConfig = { url: '', key: '' }
      data?.forEach((item) => {
